...
# [**Hans Tananda**](https://github.com/hanstananda)
# [**Aldo Halim**](https://github.com/aldo2811)
...
parameters['name'] = "self-trained-model-cnn3"  # Model name
...
class FinalNN(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim,
        ...
        self.word_cnn_layer = nn.Conv1d(in_channels=1, out_channels=2,
                                        kernel_size=(embedding_dim + self.out_channels) * 2,
                                        padding=(3 * (embedding_dim + self.out_channels) - 1) // 2)

        self.word_cnn2_layer = nn.Conv1d(in_channels=2, out_channels=4,
                                         kernel_size=(embedding_dim + self.out_channels) * 4,
                                         padding=(3 * (embedding_dim + self.out_channels) - 1) // 2)

        self.word_cnn3_layer = nn.Conv1d(in_channels=4, out_channels=2 * hidden_dim,
                                         kernel_size=(embedding_dim + self.out_channels) * 3,
                                         stride=embedding_dim + self.out_channels,
                                         padding=(embedding_dim + self.out_channels))
    ...
    def _get_cnn_features(self, sentence, chars2, chars2_length, d):
        ...
        cnn_out = self.word_cnn2_layer(cnn_out)

        cnn_out = self.word_cnn3_layer(cnn_out)
        ...
        cnn_out = self.hidden2tag(cnn_out)
